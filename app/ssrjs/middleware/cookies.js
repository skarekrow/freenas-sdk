...
var myCookies = {
  obtain: function obtain(cookieName) {
    if (typeof cookieName === "undefined") {
      return null;
    }
    return decodeURIComponent(document.cookie.replace(new RegExp("(?:(?:^|.*;)\\s*" + encodeURIComponent(cookieName).replace(/[\-\.\+\*]/g, "\\$&") + "\\s*\\=\\s*([^;]*).*$)|^.*$"), "$1")) || null;
  },
  add: function add(cookieName, cookieContent, cookieMaxAge, cookiePath, cookieDomain, cookieSecure) {
    // cookieMaxAge is to be specified in seconds
    if (typeof cookieName === "undefined" || /^(?:max\-age|path|domain|secure)$/i.test(cookieName)) {
      return false;
    }
  ...
  "delete": function _delete(cookieName, cookiePath, cookieDomain) {
    if (!this.obtain(cookieName)) {
      return false;
    }
